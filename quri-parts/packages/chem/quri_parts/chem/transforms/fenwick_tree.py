...
class FenwickNode:
    ...
    parent = None
    children: list["FenwickNode"] = list()
    index = None

    def __init__(
        self,
        parent: Union["FenwickNode", None],
    ...
    def get_ancestors(self) -> list["FenwickNode"]:
...
class FenwickTree:
    ...
    Simulation of Fermionic Models (arXiv:1701.07072) for a reference to
    the update set (U), the parity set (P) and the children set (F) sets
    of the Fenwick.
    """

    # Root node.
    root: Union[FenwickNode, None] = None
    ...
    def __init__(self, n_qubits: int):
        ...
        def fenwick(left: int, right: int, parent: FenwickNode) -> None:
    ...
    def get_node(self, j: int) -> FenwickNode:
    ...
    def get_update_set(self, j: int) -> list[FenwickNode]:
    ...
    def get_children_set(self, j: int) -> list[FenwickNode]:
    ...
    def get_remainder_set(self, j: int) -> list[FenwickNode]:
        ...
        for a in ancestors:
            for c in a.children:
                if c.index is not None and c.index < j:
                    result.append(c)

        return result

    def get_parity_set(self, j: int) -> list[FenwickNode]:
