...
def _build_circuit_qc4_d3(params_list: Sequence[float]) -> QuantumCircuit:
    ...
    gates.extend([RXX((0,1), np.pi/2), RZ(0, tz0), RZ(1, tz1), RXX((0, 1), -np.pi/2)])
    tz2, tz3 = params_list[2:4]
    gates.extend([RXX((2,3), np.pi/2), RZ(2, tz2), RZ(3, tz3), RXX((2, 3), -np.pi/2)])
    tz1, tz2 = params_list[4:6]
    gates.extend([RXX((1,2), np.pi/2), RZ(1, tz1), RZ(2, tz2), RXX((1, 2), -np.pi/2)])
