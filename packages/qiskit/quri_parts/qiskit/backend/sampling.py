...
class QiskitIdealBackend(IdealSamplingBackend):
    """A Qiskit backend for ideal simulation.
    ...
    def run(self, circuit: NonParametricQuantumCircuit) -> SamplingJob:
        ...
        jobs: list[QiskitSamplingJob] = []
        try:
            qiskit_job = self._backend.run(transpiled_circuit, **self._run_kwargs)
            qiskit_sampling_job = QiskitSamplingJob(qiskit_job)

            # Saving mode
            if self._save_data_while_sampling:
                # Re-using the save feature without shots.
                self._saved_data.append((circuit_qasm_str, 0, qiskit_sampling_job))
            jobs.append(qiskit_sampling_job)
