...
def generate_device_property(device: AwsDevice) -> DeviceProperty:
    name = device.provider_name.lower()
    if name == "ionq":
        return ionq_device_property(device)
    else:
        raise NotImplementedError(f"Unsupported provider name: {name}")


def ionq_device_property(device: AwsDevice) -> DeviceProperty:
