...
h2o_active_space_mo = ActiveSpaceMolecularOrbitals(
    h2o_mo, ActiveSpace(n_active_ele=6, n_active_orb=4)
)
...
def test_casci_result() -> None:
    ...
    assert allclose(
        pyscf_active_space_integrals.mo_1e_int.array,
        active_space_integrals.mo_1e_int.array,
    )
    assert allclose(
